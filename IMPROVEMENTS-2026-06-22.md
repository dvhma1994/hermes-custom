...
keyword ranking. Surfaced to the model via an additive injection in
`hermes_cli/tools_config._get_platform_tools` (mirrors the `context_engine` pattern — double-gated, never
touches the platform composite/recovery loop) + a `CONFIGURABLE_TOOLSETS` entry. When no `db` is handed to
the handler it uses `SemanticRecall.for_default()` (the default session DB's sidecar) so hybrid works.
**Status: LIVE** — verified in a real `hermes -z` session returning `mode:"hybrid"` with correct
meaning-based ranking; off-state keeps the default tool list byte-identical.
...
### Expert escalation — `tools/consult_expert_tool.py`  ·  `HERMES_EXPERT_CONSULT=1`
`consult_expert(question, context)` lets the default (cheaper/faster) agent escalate a genuinely hard
reasoning/coding sub-problem to a stronger expert model (Sakana **fugu-ultra**, via the `sakana`
OpenAI-compatible provider) and get its answer — run a cheap model by default, tap a frontier model only for
the hard parts. Flag-gated + fail-open + offered via the same runtime `check_fn` injection as skill_search
(default tool list byte-identical when off). LIVE-verified end-to-end (default agent → tool → fugu-ultra →
answer). The Fugu provider is configured in the local `config.yaml` (outside the repo).
