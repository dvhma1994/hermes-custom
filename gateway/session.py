...
class SessionStore:
    ...
    def _migration_complete(self) -> bool:
        """RT-01 Phase 3: return True if state.db migration has unified the session index."""
        if self._db is None or self._db_path is None:
            return False
        try:
            return self._db.migration_is_complete()
        except Exception as e:
            logger.debug("migration_is_complete check failed: %s", e)
            return False

    def _persist_to_state_db(self, session_key: str, entry: SessionEntry) -> bool:
        """RT-01 Phase 3: persist a single gateway entry to state.db."""
        if self._db is None:
            return False
        try:
            self._db.upsert_gateway_session(
            ...
            # Sync resume/suspended state
            if entry.resume_pending:
                self._db.set_resume_pending(
                    entry.session_id, reason=entry.resume_reason or "restart_timeout"
                )
            else:
                self._db.clear_resume_pending(entry.session_id)
            if entry.suspended:
                self._db.set_suspended(entry.session_id)
            return True
        except Exception as e:
            logger.debug("_persist_to_state_db failed: %s", e)
            return False
    ...
    def _ensure_loaded_locked(self) -> None:
        ...
        # RT-01 Phase 3: if migration is complete, load from state.db gateway_session_index.
        if self._migration_complete():
            try:
                rows = self._db.list_gateway_sessions()
                for row in rows:
                    entry = self._gateway_row_to_entry(row)
                    if entry:
                        self._entries[row["session_key"]] = entry
                self._loaded = True
                return
            except Exception as e:
                logger.warning("[gateway] Failed to load gateway session index from state.db: %s", e)
                # fall through to legacy sessions.json load
    ...
    def _gateway_row_to_entry(self, row: Dict[str, Any]) -> Optional[SessionEntry]:
        """RT-01 Phase 3: convert a gateway_session_index row to SessionEntry."""
        try:
            resume = self._db.get_resume_state(row["session_id"]) if self._db else None
            return SessionEntry(
                ...
                resume_pending=bool(resume.get("resume_pending")) if resume else False,
                resume_reason=resume.get("resume_reason") if resume else None,
                last_resume_marked_at=(
                    datetime.fromtimestamp(resume["last_resume_marked_at"])
                    if resume and resume.get("last_resume_marked_at")
                    else None
                ),
                suspended=bool(resume.get("suspended")) if resume else False,
            )
        except Exception as e:
            logger.debug("_gateway_row_to_entry failed for %s: %s", row.get("session_key"), e)
            return None

    def _save(self) -> None:
        """Save sessions index to the authoritative store."""
        # RT-01 Phase 3: state.db is the source of truth after migration.
        if self._migration_complete():
            try:
                for key, entry in self._entries.items():
                    self._persist_to_state_db(key, entry)
                return
            except Exception as e:
                logger.warning("[gateway] state.db persist failed, falling back to sessions.json: %s", e)
                # fall through to legacy sessions.json write

        # Legacy sessions.json write path (pre-migration or fallback)
        import tempfile
        self.sessions_dir.mkdir(parents=True, exist_ok=True)
        sessions_file = self.sessions_dir / "sessions.json"
