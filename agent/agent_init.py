...
def init_agent(
    ...
    # Flag-gated model router (HERMES_MODEL_ROUTER=1): if NO chain was configured,
    # auto-populate it from the models available to the primary's provider, so the
    # proven failure-only fallback path has a backup when the primary overloads
    # (e.g. glm-5.2 429s). No-op when disabled or a chain is already configured.
    try:
        from agent.model_router import maybe_populate_fallback_chain as _mpfc
        _mpfc(agent)
    except Exception:
        pass
