...
def compress_context(
    ...
    *,
    ...
    # Durable file-mutation ledger — re-injected IN FULL across the compression
    # boundary so the model does not recreate or re-edit files it already
    # finished earlier in this task. TodoStore drops completed items and the
    # source transcript is now gone, so without this the working-tree memory is
    # lost exactly here. Read under the CURRENT (pre-rotation) session id;
    # carry_forward below re-keys the rows to the new id for the next cycle.
    # Gated on HERMES_EXEC_LEDGER; fail-open so compression never breaks.
    try:
        if os.getenv("HERMES_EXEC_LEDGER") == "1" and agent._session_db:
            ledger_snapshot = agent._session_db.format_execution_ledger_for_injection(
                agent.session_id
            )
            if ledger_snapshot:
                compressed.append({"role": "user", "content": ledger_snapshot})
            # Verification history (the second half): what was already checked, with
            # a STALE marker + re-run reminder so a pre-compression pass never
            # licenses skipping re-verification after a later edit.
            verify_snapshot = agent._session_db.format_verification_for_injection(
                agent.session_id
            )
            if verify_snapshot:
                compressed.append({"role": "user", "content": verify_snapshot})
    except Exception:
        pass
    ...
    if agent._session_db:
        try:
            ...
            # Carry the durable execution ledger across the session-id rotation so
            # the next compression cycle's injection (which looks up the new id)
            # still sees the full file-mutation history. No FK on the table, so
            # re-keying before the new session row is created is safe. Fail-open.
            try:
                if os.getenv("HERMES_EXEC_LEDGER") == "1" and agent._session_db:
                    agent._session_db.carry_forward_execution_ledger(
                        old_session_id, agent.session_id
                    )
                    agent._session_db.carry_forward_verification_ledger(
                        old_session_id, agent.session_id
                    )
            except Exception:
                pass
