...
def run_conversation(
    ...
    # OPVAL HK01: reset any stale recorder from a previous turn on this agent
    # instance, then initialize a new recorder if OPVAL is enabled.
    agent._opval_recorder = None
    agent._opval_turn_start_time: Optional[float] = None
    agent._opval_turn_tokens_start: int = 0
    if os.getenv("HERMES_OPVAL") == "1" and not getattr(agent, "_opval_disabled_for_session", False):
        try:
            from agent.opval.integration import OpvalSessionRecorder, default_store_path
            from agent.opval.store import OpvalStore
            # Prefer the agent's existing SessionDB connection (single-connection invariant).
            _opval_conn = getattr(getattr(agent, "_session_db", None), "_conn", None)
            if _opval_conn is not None:
                _opval_store = OpvalStore(_opval_conn)
            else:
                _opval_store = OpvalStore(default_store_path())
            # Explicit synthetic detection: gateway resume, cron, /steer continuations.
            _opval_synthetic = bool(
                original_user_message is None
                or str(original_user_message).strip() == ""
                or str(original_user_message).strip().startswith("[System note:")
            )
            _opval_synthetic_reason = None
            if _opval_synthetic:
                _opval_synthetic_reason = "system_note_or_empty"
            elif getattr(agent, "_gateway_resume_pending", False):
                _opval_synthetic = True
                _opval_synthetic_reason = "gateway_resume"
            elif os.getenv("HERMES_CRON") == "1":
                _opval_synthetic = True
                _opval_synthetic_reason = "cron"
            elif getattr(agent, "_opval_steer_continuation", False):
                _opval_synthetic = True
                _opval_synthetic_reason = "steer_continuation"
            agent._opval_recorder = OpvalSessionRecorder(
                store=_opval_store,
                session_id=agent.session_id or "",
                task_id=effective_task_id or "",
                platform=getattr(agent, "platform", "") or "cli",
                anchor_message=str(original_user_message)[:2000],
                synthetic=_opval_synthetic,
                synthetic_reason=_opval_synthetic_reason,
            )
            agent._opval_turn_start_time = time.time()
            agent._opval_turn_tokens_start = getattr(agent, "session_total_tokens", 0) or 0
        except Exception as _opval_err:
            logger.warning("OPVAL HK01 init failed for session %s: %s", agent.session_id, _opval_err)
            agent._opval_recorder = None
            agent._opval_disabled_for_session = True

    # ── Milestone 1: Runtime Authority + Feedback ─────────────────────
    # Initialize lightweight per-turn authority/feedback state. These are
    # no-op by default and are only activated if external code later injects
    # an AuthorityDecision. Failures here are caught and logged so the core
    # turn cannot be disrupted.
    try:
    ...
    except Exception as _m1_init_err:
        logger.debug("Milestone 1 authority/feedback init failed: %s", _m1_init_err)

    # Learning Governance: apply learned authority once per session, here at the
    # cache-safe window (before the system prompt / tool list are built below).
    # Must run AFTER ensure_runtime_authority so the learned policy becomes the
    # per-turn baseline rather than being reset away. Flag-gated on
    # HERMES_LEARNING=1; fully no-op (and no DB touch) otherwise, and never
    # disrupts the turn. Only cache-safe authority knobs are applied.
    try:
        from agent.learning_cycle import learning_enabled, apply_learned_authority, recent_domain

        if learning_enabled() and not getattr(agent, "_learned_authority_applied", False):
            agent._learned_authority_applied = True
            _learn_conn = getattr(getattr(agent, "_session_db", None), "_conn", None)
            if _learn_conn is not None:
                apply_learned_authority(_learn_conn, agent, recent_domain(_learn_conn))
    except Exception as _learn_apply_err:
        logger.debug("learned authority application skipped: %s", _learn_apply_err)
    ...
    agent._dod_gate_fired = False  # Definition-of-Done gate: one re-prompt per turn
    agent._regression_gate_fired = False  # regression-test gate: one re-prompt per turn
    agent._todo_gate_fired = False  # plan-completion gate: one re-prompt per turn
    agent._todo_touched_this_turn = False  # plan-completion gate fires only for THIS task's plan
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # Apply authority context-size override before compression, if any.
        # This only affects the compressor threshold; it does not alter the
        # underlying model context. Failures fall back to existing behavior.
        _authority_ctx = getattr(agent, "_authority_context_size_override", None)
        if _authority_ctx is not None:
            try:
                _compressor = agent.context_compressor
                if _compressor is not None and getattr(_compressor, "context_length", None):
                    _compressor.update_model(
                        model=agent.model,
                        context_length=min(_authority_ctx, _compressor.context_length),
                        base_url=agent.base_url,
                        api_key=getattr(agent, "api_key", ""),
                        provider=agent.provider,
                        api_mode=agent.api_mode,
                    )
                    logger.debug("Applied authority context_size override: %s", _authority_ctx)
            except Exception as _ctx_err:
                logger.debug("Authority context_size override failed: %s", _ctx_err)
        ...
        # Milestone 1: inject authority tool_guidance / system_prompt_suffix.
        # Applied only if authority state is explicitly set and values are
        # non-empty. Failures are caught and do not break system-prompt build.
        try:
            _authority_guidance = getattr(agent, "_authority_tool_guidance", None)
            _authority_suffix = getattr(agent, "_authority_system_prompt_suffix", None)
            if _authority_guidance:
                effective_system = (effective_system + "\n\n[Tool guidance]\n" + str(_authority_guidance)).strip()
            if _authority_suffix:
                effective_system = (effective_system + "\n\n" + str(_authority_suffix)).strip()
        except Exception as _guidance_err:
            logger.debug("Authority guidance/suffix injection failed: %s", _guidance_err)
        if effective_system:
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                assistant_message.tool_calls = agent._cap_delegate_task_calls(
                    assistant_message.tool_calls, agent
                )
                ...
                _osc_msgs_before = len(messages)
                agent._execute_tool_calls(assistant_message, messages, effective_task_id, api_call_count)

                # ── Tool-failure oscillation guard (HERMES_OSC_GUARD) ──────────
                # API/transport errors get structured recovery (error_classifier);
                # tool-RESULT failures (a test/build/patch that keeps failing) only
                # get handed back to the model, which can loop fail->tweak->same-fail
                # until it silently burns the whole iteration budget. Detect the same
                # failure recurring across iterations and inject a one-time
                # "diagnose, don't retry" nudge. Append-only (cache-safe); fail-open.
                # Skipped when a guardrail halt already ended the turn — the nudge
                # ("go make a different change") would contradict the halt message.
                if os.getenv("HERMES_OSC_GUARD") == "1" and agent._tool_guardrail_halt_decision is None:
                    try:
                        from agent.tool_failure_oscillation import get_osc_tracker
                        _osc = get_osc_tracker(agent)
                        for _m in messages[_osc_msgs_before:]:
                            if isinstance(_m, dict) and _m.get("role") == "tool":
                                _osc.note(_m.get("tool_name") or _m.get("name"), _m.get("content"))
                        _osc_nudge = _osc.flush_iteration()
                        if _osc_nudge:
                            messages.append({"role": "user", "content": _osc_nudge})
                            logger.info(
                                "Tool-failure oscillation nudge injected (session=%s)",
                                agent.session_id,
                            )
                    except Exception as _osc_err:
                        logger.debug("oscillation guard skipped: %s", _osc_err)

                # Milestone 1: apply authority feedback adaptation after tool
                # execution. Only allowed authority knobs may be affected.
                # Failures are caught and do not break the turn loop.
                try:
                    if hasattr(agent, "_runtime_feedback") and hasattr(agent, "_runtime_authority"):
                        _authority_updates = agent._runtime_feedback.adapt(agent._runtime_authority)
                        if _authority_updates is not None:
                            from agent.runtime_authority import apply_decision
                            apply_decision(agent, _authority_updates)
                            logger.debug("Applied authority feedback adaptation")
                except Exception as _adapt_err:
                    logger.debug("Authority feedback adaptation failed: %s", _adapt_err)
                ...
                if _tc_names == {"execute_code"}:
                    agent.iteration_budget.refund()
                # Mark the plan as touched THIS turn so the plan-completion gate
                # only enforces a list the agent is actively working on — not a
                # stale list rehydrated from an earlier, finished task.
                if "todo" in _tc_names:
                    agent._todo_touched_this_turn = True
                ...
                # The verification ledger knows which checks are FAIL/STALE, but the
                # DoD is otherwise just prose the model can ignore — so it can finish
                # "done" with a red or unverified check, which a senior reviewer
                # rejects on first read. Before accepting the final answer, if a
                # recorded check is FAIL or STALE, re-prompt ONCE to force a green
                # re-verification (or an honest BLOCKED). Append-only (cache-safe,
                # same shape as the codex-ack continuation above); fail-open; bounded
                # to one re-prompt per turn so it can never loop.
                if (
                    os.getenv("HERMES_DOD_GATE") == "1"
                    and not getattr(agent, "_dod_gate_fired", False)
                    and agent._session_db is not None
                    and final_response.strip()
                ):
                    try:
                        _dod_open = agent._session_db.get_open_verification_failures(agent.session_id)
                    except Exception:
                        _dod_open = []
                    if _dod_open:
                        agent._dod_gate_fired = True
                        _dod_items = "; ".join(
                            f"`{o['command']}`" + (" (FAIL)" if o.get("passed") == 0 else " (STALE)")
                            for o in _dod_open[:8]
                        )
                        messages.append({
                            "role": "user",
                            "content": (
                                "[System: do NOT finish yet — your Definition of Done is not met. "
                                f"These checks are unresolved: {_dod_items}. Re-run them now: if a "
                                "check FAILS, fix the root cause; if STALE (a file changed after it "
                                "last ran), re-run to confirm the current code is green. Give your "
                                "final answer only once they pass. If a failure is genuinely "
                                "unfixable, reply BLOCKED and explain exactly what and why.]"
                            ),
                        })
                        agent._session_messages = messages
                        logger.info(
                            "DoD gate re-prompt: %d open check(s) (session=%s)",
                            len(_dod_open), agent.session_id,
                        )
                        continue

                # ── Regression-test gate (HERMES_REGRESSION_GATE) ──────────────
                # The DoD gate proves tests RAN green; this asks "where's the test?"
                # — a fix that changed source but added no test ships unprotected,
                # the canonical first-read reviewer rejection. One bounded re-prompt
                # per turn; append-only (cache-safe); fail-open.
                if (
                    os.getenv("HERMES_REGRESSION_GATE") == "1"
                    and not getattr(agent, "_regression_gate_fired", False)
                    and agent._session_db is not None
                    and final_response.strip()
                ):
                    try:
                        _untested = agent._session_db.get_open_test_coverage_gap(agent.session_id)
                    except Exception:
                        _untested = []
                    if _untested:
                        agent._regression_gate_fired = True
                        _files = ", ".join(_untested[:6])
                        messages.append({
                            "role": "user",
                            "content": (
                                "[System: you changed code but added or modified NO test this task "
                                f"(changed: {_files}). Add a regression test that FAILS without your "
                                "change and PASSES with it, then run it — or state explicitly why no "
                                "test is warranted (e.g. a pure refactor already covered by existing "
                                "tests). Then give your final answer.]"
                            ),
                        })
                        agent._session_messages = messages
                        logger.info(
                            "Regression gate re-prompt: %d untested source change(s) (session=%s)",
                            len(_untested), agent.session_id,
                        )
                        continue

                # ── Plan-completion gate (HERMES_TODO_GATE) ─────────────────────
                # The DoD/regression gates check verification + tests; this checks
                # the agent's OWN plan: finishing with pending/in_progress todos is
                # the "did X of X,Y,Z then stopped" abandonment, and it fires on
                # tasks the other gates miss (no checks, no source/test delta). One
                # re-prompt/turn; append-only (cache-safe); fail-open; no DB needed.
                if (
                    os.getenv("HERMES_TODO_GATE") == "1"
                    and not getattr(agent, "_todo_gate_fired", False)
                    and getattr(agent, "_todo_touched_this_turn", False)
                    and final_response.strip()
                ):
                    try:
                        _store = getattr(agent, "_todo_store", None)
                        _open_todos = _store.open_items() if _store else []
                    except Exception:
                        _open_todos = []
                    if _open_todos:
                        agent._todo_gate_fired = True
                        _items = "; ".join(
                            f"[{t.get('status')}] {str(t.get('content', ''))[:100]}"
                            for t in _open_todos[:8]
                        )
                        messages.append({
                            "role": "user",
                            "content": (
                                "[System: do NOT finish yet — your own task list still has open items: "
                                f"{_items}. Complete them now, or mark each cancelled with a brief reason "
                                "if it is genuinely out of scope, then give your final answer.]"
                            ),
                        })
                        agent._session_messages = messages
                        logger.info(
                            "Todo gate re-prompt: %d open plan item(s) (session=%s)",
                            len(_open_todos), agent.session_id,
                        )
                        continue
    ...
    # OPVAL HK02 + HK03: single post-loop capture, then session finalize.
    # Strictly observational. Failure here is logged and ignored.
    # Runs BEFORE finalize_turn and does not modify its signature or return value.
    _opval_recorder = getattr(agent, "_opval_recorder", None)
    if _opval_recorder is not None:
        try:
            # HK02: Extract tool calls and outputs from messages list.
            # Only tools that actually produced a role='tool' message are captured.
            _opval_tool_calls: List[str] = []
            _opval_tool_outputs: List[Dict[str, Any]] = []
            for _m in messages:
                ...
                if _m.get("role") == "assistant" and _m.get("tool_calls"):
                    for _tc in _m["tool_calls"]:
                        if isinstance(_tc, dict):
                            ...
                            if _name:
                                _opval_tool_calls.append(str(_name))
                        elif isinstance(_tc, str):
                            _opval_tool_calls.append(_tc)
                elif _m.get("role") == "tool":
                    _content = str(_m.get("content", ""))
                    _is_error = (
                        "Error executing tool" in _content
                        or "Tool blocked" in _content
                        or "Tool execution cancelled" in _content
                        or "cancelled" in _content.lower()
                    )
                    _opval_tool_outputs.append({
                        "exit_code": 1 if _is_error else 0,
                        "error": _content[:500] if _is_error else None,
                        "name": _m.get("name", ""),
                        "tool_call_id": _m.get("tool_call_id", ""),
                    })
            _opval_recorder.record_turn(
                user_message=str(original_user_message or user_message)[:2000],
                agent_message=str(final_response or "")[:2000],
                tool_calls=_opval_tool_calls,
                tool_outputs=_opval_tool_outputs,
                interrupted=interrupted,
                clarify=any(t == "clarify" for t in _opval_tool_calls),
                latency_ms=int((time.time() - (agent._opval_turn_start_time or time.time())) * 1000),
                tokens_used=(getattr(agent, "session_total_tokens", 0) or 0) - agent._opval_turn_tokens_start,
                checkpoint_event=_turn_exit_reason,
            )
            # HK03: Update session_id if compression/rotation changed it mid-turn.
            _final_session_id = agent.session_id or _opval_recorder._session_id
            _opval_recorder._session_id = _final_session_id
            # HK03: Build lineage using the agent's existing SessionDB instance.
            _opval_parent: Optional[str] = None
            _opval_db = getattr(agent, "_session_db", None)
            if _opval_db is not None:
                try:
                    _chain = _opval_db._session_lineage_root_to_tip(_final_session_id)
                    if _chain and len(_chain) > 1:
                        _opval_parent = _chain[-2]
                except Exception:
                    pass
            # HK03: Finalize the session record.
            _opval_outcome = (
                "success"
                if final_response is not None and not failed and not interrupted
                else ("interrupt" if interrupted else "error")
            )
            _opval_synthetic = bool(getattr(_opval_recorder, "_synthetic", False))
            _opval_recorder.finalize(
                outcome=_opval_outcome,
                parent_session_id=_opval_parent,
                session_id=_final_session_id,
                synthetic=_opval_synthetic,
            )
            # HK03: Close the recorder (drops TurnCollector reference). Do NOT
            # close the store here because it may be the shared SessionDB connection.
            try:
                _opval_recorder.close()
            except Exception:
                pass
        except Exception as _opval_err:
            logger.warning("OPVAL HK02/HK03 failed: %s", _opval_err)
        finally:
            # Always clear the agent attribute to prevent a stale recorder
            # from being reused if compression/rotation created a new session.
            agent._opval_recorder = None

    # Learning Governance (autonomous digest): turn accumulated OPVAL evidence
    # into governance decisions at session end. Placed OUTSIDE the OPVAL-recorder
    # block so it fires in every session-end context (the digest reads the store
    # directly and does not need this turn's recorder). Throttled to once per
    # interval, exception-safe, and flag-gated on HERMES_LEARNING=1 so it is a
    # complete no-op (no DB touch) when disabled.
    try:
        from agent.learning_cycle import learning_enabled, maybe_run_session_end_digest

        if learning_enabled():
            # Prefer the agent's SessionDB connection; the digest falls back to
            # OPVAL's default store when none is attached.
            _digest_conn = getattr(getattr(agent, "_session_db", None), "_conn", None)
            maybe_run_session_end_digest(_digest_conn)
    except Exception as _digest_err:
        logger.debug("learning digest skipped: %s", _digest_err)
