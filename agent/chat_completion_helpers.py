...
def _maybe_filter_tools_for_authority(agent, tools_for_api: list) -> list:
    """Filter tool definitions by runtime authority tool_scope if active."""
    scope = getattr(agent, "_authority_tool_scope", None)
    if scope is None:
        return tools_for_api
    try:
        from agent.runtime_authority import filter_tool_scope
        return filter_tool_scope(tools_for_api, scope)
    except Exception as exc:
        logger.warning(
            "%s⚠️ Tool-scope filter failed; falling back to full tool set: %s",
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    tools_for_api = _maybe_filter_tools_for_authority(agent, agent.tools)
