...
class AIAgent:
    ...
    @staticmethod
    def _cap_delegate_task_calls(tool_calls: list, agent: Any = None) -> list:
        ...
        Milestone 1: additionally enforces agent._authority_delegation_policy
        when set to "no_self_delegate" by stripping delegate_task calls
        entirely. Other policies are accepted but do not alter behavior yet.

        Returns the original list if no truncation was needed.
        """
        from tools.delegate_tool import _get_max_concurrent_children

        # Milestone 1: authority delegation policy enforcement.
        if agent is not None:
            policy = getattr(agent, "_authority_delegation_policy", None)
            if policy == "no_self_delegate":
                filtered = [tc for tc in tool_calls if tc.function.name != "delegate_task"]
                removed = len(tool_calls) - len(filtered)
                if removed:
                    logger.warning(
                        "Removed %d delegate_task call(s) due to authority policy '%s'",
                        removed, policy,
                    )
                return filtered
    ...
    def _append_guardrail_observation(
        ...
        *,
        ...
        _store = getattr(self, "_todo_store", None)
        _has_plan: bool | None = None
        if _store is not None:
            try:
                _has_plan = bool(_store.has_items())
            except Exception:
                _has_plan = None
