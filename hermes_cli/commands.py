...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    # Maintenance (RT-01 Phase 1)
    CommandDef("repair-state-db", "Diagnose and repair state.db safely (snapshot + dry-run + confirmation)",
               "Maintenance", cli_only=True,
               args_hint="[--dry-run|--force|--status]",
               subcommands=("--dry-run", "--force", "--status")),
    CommandDef("restore-state-db", "Restore state.db from a validated backup", "Maintenance", cli_only=True,
               args_hint="<backup-path> [--force]", subcommands=("--force",)),
]
