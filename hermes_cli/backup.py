...
def _format_size(nbytes: int) -> str:
    ...
    The unit is chosen by magnitude, then the rounded value is re-checked
    against the 1024 boundary.  Without that re-check a value just below the
    next unit (e.g. 1048575 = 1 MB - 1 byte) rounds up to "1024.0 KB" instead
    of rolling over into "1.0 MB".
    """
    units = ("B", "KB", "MB", "GB", "TB")
    size = float(nbytes)
    idx = 0
    while size >= 1024 and idx < len(units) - 1:
        size /= 1024.0
        idx += 1
    if idx > 0 and round(size, 1) >= 1024 and idx < len(units) - 1:
        # Rounding pushed the value onto the next unit boundary.
        size /= 1024.0
        idx += 1
    unit = units[idx]
    if unit == "B":
