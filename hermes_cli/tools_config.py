...
CONFIGURABLE_TOOLSETS = [
    ...
    ("skill_search",    "🧭 Skill Search",              "find a skill by meaning (requires HERMES_SEMANTIC_RECALL)"),
    ("consult_expert",  "🧠 Consult Expert",            "escalate hard sub-problems to fugu-ultra (requires HERMES_EXPERT_CONSULT)"),
...
def _get_platform_tools(
    ...
    *,
    ...
    # Semantic skill_search is a flag-gated, registry-only one-tool toolset (not part of
    # any static platform composite, like context_engine above). Surface it whenever its
    # check_fn passes (HERMES_SEMANTIC_RECALL=1 + skills available), unless the user chose
    # an explicit empty selection. Double-gated: registry.get_definitions filters by the
    # same check_fn, so even a stray add cannot alter the default tool list / prompt cache.
    if not explicit_empty_selection:
        try:
            from tools.skill_search_tool import check_skill_search_requirements
            if check_skill_search_requirements():
                enabled_toolsets.add("skill_search")
        except Exception:
            pass

    # consult_expert (flag-gated registry-only one-tool toolset): lets the agent escalate a
    # hard sub-problem to a stronger expert model. Same additive + double-gated pattern as
    # skill_search above (check_fn gates HERMES_EXPERT_CONSULT=1).
    if not explicit_empty_selection:
        try:
            from tools.consult_expert_tool import check_consult_expert_requirements
            if check_consult_expert_requirements():
                enabled_toolsets.add("consult_expert")
        except Exception:
            pass
