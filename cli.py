...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def _show_session_status(self):
        ...
        # RT-01 Phase 1: surface safe mode in /status
        if self._session_db and getattr(self._session_db, "safe_mode", None) and self._session_db.safe_mode.active:
            lines.extend([
                "",
                "[red bold]STATE.DB SAFE MODE ACTIVE[/]",
                f"[red]{self._session_db.safe_mode.reason}[/]",
                "Run /repair-state-db --dry-run for diagnosis.",
            ])
    ...
    def _handle_repair_state_db(self, cmd_original: str):
        """RT-01 Phase 1: explicit state.db repair CLI."""
        from hermes_state_backup import RepairCLIContractor
        from hermes_state import DEFAULT_DB_PATH

        args = cmd_original.strip().split()[1:]
        force = "--force" in args
        dry_run = "--dry-run" in args
        show_status = "--status" in args

        db_path = getattr(self, "_session_db", None) and self._session_db.db_path
        db_path = db_path or DEFAULT_DB_PATH

        if show_status:
            contractor = RepairCLIContractor(db_path)
            report = contractor._generate_integrity_report()
            lines = ["state.db integrity status:"]
        ...
        contractor = RepairCLIContractor(db_path)
        report = contractor.run_repair(force=force, dry_run=dry_run)

        lines = ["state.db repair report:"]
        lines.append(f"  dry_run: {report.get('dry_run', False)}")
        if report.get("snapshot"):
            lines.append(f"  snapshot: {report['snapshot'].get('path')}")
        lines.append(f"  actions_taken: {report.get('actions_taken', [])}")
        if report.get("integrity_before"):
            lines.append(f"  integrity_before: {report['integrity_before'].get('quick_check')}")
        if report.get("integrity_after"):
            lines.append(f"  integrity_after: {report['integrity_after'].get('quick_check')}")
        if report.get("error"):
            lines.append(f"  [red]error: {report['error']}[/]")
        if report.get("rollback_instructions"):
            lines.append(f"  rollback: {report['rollback_instructions']}")
        self._console_print("\n".join(lines), highlight=False, markup=False)

        # Reopen DB so CLI continues to work if repair succeeded
        if not report.get("error") and not dry_run:
            try:
                from hermes_state import SessionDB
                self._session_db = SessionDB()
            except Exception as exc:
                self._console_print(f"[red]Could not reopen state.db after repair: {exc}[/]")

    def _handle_restore_state_db(self, cmd_original: str):
        """RT-01 Phase 1: explicit state.db restore CLI."""
        from hermes_state_backup import StateDBBackupManager
        from hermes_state import DEFAULT_DB_PATH

        args = cmd_original.strip().split()[1:]
        if not args or args[0].startswith("-"):
            self._console_print("Usage: /restore-state-db \u003cbackup-path\u003e [--force]")
            return
        ...
        if not force:
            self._console_print(
                f"This will replace {db_path} with {backup_path}. "
                "The current DB will be quarantined. Add --force to proceed."
            )
            return
        ...
        mgr = StateDBBackupManager(db_path)
        result = mgr.restore_from_backup(backup_path)

        if result["success"]:
            self._console_print(f"Restored state.db from {backup_path}")
            self._console_print(f"Quarantined old DB at {result.get('quarantine')}")
            try:
                from hermes_state import SessionDB
                self._session_db = SessionDB()
            except Exception as exc:
                self._console_print(f"[red]Could not reopen state.db after restore: {exc}[/]")
        else:
            self._console_print(f"[red]Restore failed: {result.get('error')}[/]")
    ...
    def process_command(self, command: str) -> bool:
        ...
        elif canonical == "repair-state-db":
            self._handle_repair_state_db(cmd_original)
        elif canonical == "restore-state-db":
            self._handle_restore_state_db(cmd_original)
