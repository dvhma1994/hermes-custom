...
class TestPatchHandler:
    ...
    def test_patch_v4a_rejects_traversal_in_move_source(self, mock_get):
        """`*** Move File:` was previously not extracted, so its paths escaped the
        traversal/sensitive/lock guards. Source-side traversal must now be caught."""
        from tools.file_tools import patch_tool
        result = json.loads(patch_tool(
            mode="patch",
            patch=(
                "*** Begin Patch\n"
                "*** Move File: ../../../etc/shadow -> safe.py\n"
                "*** End Patch\n"
            ),
        ))
        assert "error" in result
    ...
    @patch("tools.file_tools._get_file_ops")
    def test_patch_v4a_rejects_traversal_zero_space_header(self, mock_get):
        """The parser accepts `***Update File:` with NO space after `***`; the guard
        regex must too, or that spelling bypasses the traversal check."""
        from tools.file_tools import patch_tool
        result = json.loads(patch_tool(
            mode="patch",
            patch="*** Begin Patch\n***Update File: ../../../etc/shadow\n@@\n-a\n+b\n*** End Patch\n",
    ...
    @patch("tools.file_tools._get_file_ops")
    def test_patch_v4a_rejects_traversal_zero_space_move(self, mock_get):
        from tools.file_tools import patch_tool
        result = json.loads(patch_tool(
            mode="patch",
            patch="*** Begin Patch\n***Move File: a.py -> ../../../etc/shadow\n*** End Patch\n",
        ))
        assert "error" in result
        assert "traversal" in result["error"].lower()
        mock_get.return_value.patch_v4a.assert_not_called()
    ...
    def test_patch_v4a_rejects_traversal_in_move_destination(self, mock_get):
        """The destination is the key gap — a move could write OUTSIDE the tree
        with no check. Destination-side traversal must now be caught."""
        from tools.file_tools import patch_tool
        result = json.loads(patch_tool(
            mode="patch",
            patch=(
