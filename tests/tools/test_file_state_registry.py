...
class FileStateRegistryUnitTests(unittest.TestCase):
    ...
    def test_writes_by_returns_all_task_writes_no_path_filter(self):
        foo = self._mk()
        bar = self._mk()
        since = time.time()
        time.sleep(0.01)
        file_state.note_write("child", foo)
        file_state.note_write("child", bar)
        file_state.note_write("other", self._mk())  # a different task's write
        got = file_state.writes_by("child", since)
        self.assertEqual(set(got), {foo, bar})  # all of child's, none of other's

    def test_writes_by_respects_since_and_unknown_task(self):
        p = self._mk()
        file_state.note_write("child", p)
        # A `since` in the future excludes all current writes (deterministic; no
        # reliance on clock granularity), while since=0 includes them.
        self.assertEqual(file_state.writes_by("child", time.time() + 1000), [])
        self.assertEqual(file_state.writes_by("child", 0.0), [p])
        self.assertEqual(file_state.writes_by("nobody", 0.0), [])  # unknown task -> []
