...
class TestOpenItems:
    """open_items() backs the plan-completion gate (HERMES_TODO_GATE)."""

    def test_returns_pending_and_in_progress_only(self):
        store = TodoStore()
        store.write([
        ...
        assert [t["id"] for t in store.open_items()] == ["1", "2"]

    def test_empty_when_all_done_or_cancelled(self):
        store = TodoStore()
        store.write([
    ...
    def test_empty_store(self):
        assert TodoStore().open_items() == []

    def test_returns_copies(self):
        store = TodoStore()
        store.write([{"id": "1", "content": "a", "status": "pending"}])
        store.open_items()[0]["content"] = "mutated"
        assert store.read()[0]["content"] == "a"  # original unchanged
