...
def _validate_operations(
    ...
    # Reject any path targeted by more than one operation. Apply mutates the tree
    # across ops, so a second op on the same path sees changed content AND the
    # index-keyed rollback snapshot (captured against the initial tree) would be
    # stale — a data-loss hazard. Such patches are ambiguous regardless; the model
    # should combine them into a single op (one Update/Add per file, multiple @@
    # hunks inside). Covers a MOVE's new_path colliding with another op's target.
    _seen_targets: set = set()
    _flagged: set = set()
    for op in operations:
        _targets = [op.file_path]
        if op.operation == OperationType.MOVE and op.new_path:
            _targets.append(op.new_path)
        for _t in _targets:
            if _t in _seen_targets and _t not in _flagged:
                errors.append(
                    f"{_t}: targeted by more than one operation in this patch — "
                    "combine them into a single operation for that file."
                )
                _flagged.add(_t)
...
def _capture_v4a_snapshots(operations: List[PatchOperation], file_ops: Any) -> dict:
    """Capture pre-apply state (BEFORE any write) keyed by op index, so a partial
    multi-file apply can be rolled back. ADD -> delete-on-undo; UPDATE/DELETE ->
    original content; MOVE -> reverse the rename (content-preserving)."""
    snaps: dict = {}
    for i, op in enumerate(operations):
        try:
            if op.operation == OperationType.ADD:
                snaps[i] = ("delete_created", op.file_path)
            elif op.operation in (OperationType.UPDATE, OperationType.DELETE):
                r = file_ops.read_file_raw(op.file_path)
                content = None if getattr(r, "error", None) else getattr(r, "content", None)
                snaps[i] = ("restore_content", op.file_path, content)
            elif op.operation == OperationType.MOVE:
                snaps[i] = ("undo_move", op.file_path, op.new_path)
        except Exception:
            snaps[i] = ("skip",)
    return snaps


def _rollback_v4a(applied_indices: List[int], snapshots: dict, file_ops: Any) -> Tuple[bool, str]:
    """Undo the successfully-applied ops in reverse. Returns (fully_ok, detail).
    fully_ok=False if any single restore could not complete (then the caller must
    NOT claim a clean rollback)."""
    failures: List[str] = []
    for i in reversed(applied_indices):
        snap = snapshots.get(i)
        if not snap or snap[0] == "skip":
            failures.append(f"op#{i} (no snapshot)")
            continue
        kind = snap[0]
        try:
            if kind == "delete_created":
                res = file_ops.delete_file(snap[1])
                if getattr(res, "error", None):
                    failures.append(f"{snap[1]}: {res.error}")
            elif kind == "restore_content":
                _, path, content = snap
                if content is None:
                    failures.append(f"{path} (no pre-image)")
                    continue
                res = file_ops.write_file(path, content)
                if getattr(res, "error", None):
                    failures.append(f"{path}: {res.error}")
            elif kind == "undo_move":
                _, src, dst = snap
                res = file_ops.move_file(dst, src)  # move it back
                if getattr(res, "error", None):
                    failures.append(f"{dst}->{src}: {res.error}")
...
def apply_v4a_operations(operations: List[PatchOperation],
    ...
    # Multi-file apply atomicity: capture pre-state up front so a partial apply
    # (file A written, file B then fails) can be rolled back instead of leaving a
    # half-applied tree. Gated on HERMES_V4A_ROLLBACK; `_applied` tracks indices of
    # ops that actually landed, for reverse restore on error.
    _rb_on = os.getenv("HERMES_V4A_ROLLBACK") == "1"
    _snaps = _capture_v4a_snapshots(operations, file_ops) if _rb_on else {}
    _applied: List[int] = []
    ...
    if errors:
        if _rb_on and _applied:
            rolled_ok, restore_detail = _rollback_v4a(_applied, _snaps, file_ops)
            if rolled_ok:
                # Every applied change was reverted — the tree is back to its
                # pre-patch state, so report no net file changes.
                return PatchResult(
                    success=False,
                    error="Apply phase failed; ALL partial changes were rolled back "
                          "— the repository was restored to its pre-patch state. "
                          "Fix the cause and re-apply. Errors:\n"
            ...
            return PatchResult(
                ...
                error="Apply phase failed AND automatic rollback was INCOMPLETE — "
                      "manual review required (run `git diff`). Rollback failures: "
                      f"{restore_detail}. Apply errors:\n"
                      + "\n".join(f"  • {e}" for e in errors),
            )
        return PatchResult(
            success=False,
