...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    if mode == "patch" and patch:
        ...
        # \s* (not \s+) after *** to exactly match the parser (patch_parser.py:112-115
        # use \*\*\*\s*), so `***Update File: ...` with no space can't bypass the guard.
        ...
        # `*** Move File: src -> dst` writes BOTH paths but was missed by the
        # Update/Add/Delete regex above — so a move destination escaped the
        # traversal/sensitive/cross-profile checks AND per-path locking + stale
        # detection (concurrent-subagent corruption + a TOCTOU on "dst exists").
        # Extract both endpoints so every downstream guard covers them too.
        for _m in _re.finditer(
            r'^\*\*\*\s*Move\s+File:\s*(.+?)\s*->\s*(.+?)\s*$', patch, _re.MULTILINE
        ):
            for v4a_path in (_m.group(1).strip(), _m.group(2).strip()):
                ...
                if has_traversal_component(v4a_path):
                    return tool_error(
                        f"V4A patch header contains '..' traversal: {v4a_path!r}. "
                        "Use a cwd-relative path (no '..') or an absolute path in "
                        "'*** Move File: src -> dst' headers."
                    )
                _paths_to_check.append(v4a_path)
