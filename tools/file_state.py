...
class FileStateRegistry:
    ...
    def writes_by(self, task_id: str, since_ts: float) -> List[str]:
        """Return resolved paths written by ``task_id`` at/after ``since_ts``.

        Unlike ``writes_since`` (which gates on a path allowlist and EXCLUDES a
        task), this returns ALL writes BY one task with no path filter — used to
        attribute a subagent's actual file writes back to the parent model. Note
        only ``write_file``/``patch`` record here; shell/heredoc writes via the
        terminal tool do not, so an empty result is "no tracked write," not a
        proof of "no write." Best-effort.
        """
        if _disabled():
            return []
        out: List[str] = []
