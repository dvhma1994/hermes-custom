...
def _discover(
    ...
    # Optional hybrid semantic re-ranking (opt-in via semantic=True, gated on
    # HERMES_SEMANTIC_RECALL, fail-open). Reorders the FTS5 message hits by
    # RRF(FTS, vector) so semantically-similar hits surface even without lexical
    # overlap; dedup-by-lineage below then keeps their sessions. Strictly a no-op
    # (identical FTS order) when disabled, no embedder is configured, or the vector
    # index is empty — so default behaviour is unchanged.
    if semantic:
        try:
            from agent import semantic_recall as _sr
            if _sr.is_enabled():
                # for_db routes to an ISOLATED sidecar index (never the live state.db
                # connection) keyed to this db's path → safe + per-profile correct.
                _rec = _sr.SemanticRecall.for_db(db)
                if _rec.available:
                    _by_id = {str(r["id"]): r for r in raw_results}
                    _ranked = _rec.hybrid_rank(
                        "message", query, list(_by_id.keys()), k=len(_by_id)
                    )
                    reordered = [_by_id[i] for i in _ranked if i in _by_id]
                    if reordered:
                        raw_results = reordered
        except Exception as e:
            logging.debug("semantic rerank skipped: %s", e, exc_info=True)
...
def session_search(
    ...
    # Default the hybrid semantic re-rank ON when the operator has enabled semantic
    # recall (HERMES_SEMANTIC_RECALL=1) — the model rarely sets the `semantic` arg, so
    # the populated vector index would otherwise stay dormant. This is server-side only
    # (no schema/prompt change → caching untouched); the re-rank is candidate-restricted
    # to the FTS5 hits and fully fail-open (RRF can only reorder, never returns worse).
    eff_semantic = bool(semantic)
    if not eff_semantic:
        try:
            from agent import semantic_recall as _sr
            eff_semantic = _sr.is_enabled()
        except Exception:
            eff_semantic = False
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "semantic": {
                "type": "boolean",
                "description": (
                    "Discovery shape only. When true, hybrid (keyword + vector) recall: "
                    "re-ranks FTS5 hits by semantic similarity so relevant past sessions "
                    "surface even when worded differently from your query. Requires the "
                    "operator to have enabled semantic recall (HERMES_SEMANTIC_RECALL) and "
                    "built the index; otherwise it transparently falls back to keyword-only. "
                    "Default false."
                ),
                "default": False,
            },
