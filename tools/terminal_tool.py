...
# Salience-aware truncation (HERMES_SALIENT_TRUNCATION): blind head/tail truncation
# of a verbose test/build log discards the middle — exactly where the failure
# detail (which test, the assertion, the traceback) usually lives. This recovers
# those lines from the omitted region so the verification feedback loop isn't
# corrupted on long runs.
_SALIENT_MARKERS = (
    "FAILED", "Traceback (most recent call last)", "AssertionError",
    "Error:", "error:", "error[", "error TS", "panic:", "panicked",
    "Segmentation fault", "--- FAIL", "✖", "✕",  # ✕ = U+2715 (jest), ✖ = U+2716
)
_SALIENT_LINE_RE = re.compile(
    r"^(?:"
    r"E\s"                                   # pytest failure detail line
    r"|FAIL\b"                               # go: 'FAIL\tpkg', 'FAIL: ...'
    r"|\s*\d+\s+(?:passed|failed|error)"     # '3 failed, 5 passed'
    r"|(?:=+\s*)?\d+\s+failed"               # '=== 5 failed in 2s ==='
    r"|Tests:\s"                             # jest summary 'Tests: 1 failed, ...'
    r"|\S+:\d+:\d+:?\s+error\b"              # tsc/eslint 'file:line:col error'
    r")",
    re.I,
)


def _extract_salient_lines(text: str, max_chars: int = 4000, max_lines: int = 40) -> str:
    """Pull failure/error lines out of (the omitted middle of) a long tool output.
    Line-based (no regex backtracking risk); deduped; each line capped; bounded by
    max_chars/max_lines. Returns '' when nothing salient is found."""
    if not text:
        return ""
    seen: set[str] = set()
    out: List[str] = []
    ...
    for raw in text.splitlines():
        ...
        if not line or line in seen:
            continue
        if any(mk in line for mk in _SALIENT_MARKERS) or _SALIENT_LINE_RE.match(line):
            seen.add(line)
            out.append(line)
            total += len(line) + 1
            if len(out) >= max_lines or total >= max_chars:
                break
    return "\n".join(out)


# Shell "not found" forms (sh/dash: 'sh: 1: ruff: not found') — anchored so it can't
# match prose like 'KeyError: not found' or '404: not found'.
_SHELL_NOT_FOUND_RE = re.compile(r"(?im)^(?:sh|bash|dash|zsh|ash|ksh):[^\n]*: not found$")


def _detect_env_setup_failure(command: str, output: str, exit_code: int) -> Optional[str]:
    """If a non-zero command failed for an ENVIRONMENT/SETUP reason (missing
    dependency, binary not on PATH, unprovisioned tree) rather than a code bug,
    return an actionable note so the model provisions the environment instead of
    "fixing" working code or giving up. High-precision markers only — err toward
    staying silent (and toward "check both") so a real code bug is never
    confidently mislabeled as a setup problem."""
    if not output or exit_code == 0:
        return None
    low = output.lower()
    # Python. NOTE: "No module named X" can be a missing third-party dep OR a
    # wrong/typo'd import of a LOCAL module (a real code bug), so name BOTH rather
    # than steering the model away from fixing its own import.
    if (
        "modulenotfounderror" in low or "no module named" in low
        or "could not find a version that satisfies" in low
        or "no matching distribution found" in low
        or "externally-managed-environment" in low
    ):
        return (
            "This may be an ENVIRONMENT/DEPENDENCY issue rather than a code bug — but a "
            "'No module named' can also be a wrong/typo'd import of a LOCAL module. Check "
            "BOTH: verify the import path, and that deps are installed in the right venv "
            "(`pip install -r requirements.txt` / `uv sync` / `pip install -e .`)."
        )
    if "cannot find module" in low or (
        "npm err!" in low and ("enoent" in low or "package.json" in low)
    ):
        return (
            "Likely MISSING NODE DEPENDENCIES — run `npm install` (or `pnpm`/`yarn "
            "install`) before re-running; or check the require/import path is correct."
        )
    if (
        "cannot find package" in low
        or "go: cannot find main module" in low
        or "missing go.sum entry" in low
    ):
        return (
            "Likely a Go module/dependency setup issue — run `go mod download` / "
            "`go mod tidy` before re-running."
        )
    if "could not find `cargo.toml`" in low or "error: linker" in low:
        return (
            "Likely a Rust toolchain/manifest setup issue — run from the crate root and "
            "ensure the toolchain is installed before re-running."
        )
    if (
        "command not found" in low
        or "is not recognized as an internal or external command" in low
        or ("/usr/bin/env:" in low and "no such file" in low)  # missing interpreter
        or _SHELL_NOT_FOUND_RE.search(output)
    ):
        return (
            "A required BINARY isn't on PATH (or the interpreter is missing) — install the "
            "toolchain or activate the project's venv/environment; not a code bug."
        )
    return None
    ...
    try:
        ...
        if background:
            ...
            if len(output) > MAX_OUTPUT_CHARS:
                # Reserve a slice of the budget for a salience digest so failure
                # lines in the discarded middle still reach the model (gated on
                # HERMES_SALIENT_TRUNCATION; off → original blind head/tail).
                # Only reserve digest budget when the cap is comfortably larger
                # than it, so a small configured cap can never drive head/tail
                # negative; otherwise fall back to plain head/tail.
                _salient_on = (
                    os.getenv("HERMES_SALIENT_TRUNCATION") == "1"
                    and MAX_OUTPUT_CHARS > 8000
                )
                _digest_budget = 4000 if _salient_on else 0
                _budget = MAX_OUTPUT_CHARS - _digest_budget
                head_chars = int(_budget * 0.4)  # 40% head (errors often appear early)
                tail_chars = _budget - head_chars  # 60% tail (most recent output)
                omitted = len(output) - head_chars - tail_chars
                _digest = ""
                if _salient_on:
                    # Strip ANSI BEFORE scanning: colorized pytest/jest lines start
                    # with an escape, so the ^-anchored markers would otherwise miss
                    # the exact failure lines this is meant to recover. (The whole
                    # output is ANSI-stripped again below; this is just for the scan.)
                    from tools.ansi_strip import strip_ansi as _strip_for_digest
                    _digest = _extract_salient_lines(
                        _strip_for_digest(output[head_chars: len(output) - tail_chars]),
                        max_chars=_digest_budget,
                    )
                _digest_block = (
                    f"--- salient failure lines recovered from the {omitted}-char "
                    f"omitted middle ---\n{_digest}\n\n"
                    if _digest else ""
                )
                truncated_notice = (
            ...
            # Distinguish an environment/setup failure (missing dep, binary not on
            # PATH) from a code bug, so the model provisions instead of "fixing"
            # working code. Scans the already-stripped+redacted output (incl. the
            # salient digest), so a setup error buried mid-log still surfaces.
            if os.getenv("HERMES_ENV_DIAG") == "1":
                _setup_hint = _detect_env_setup_failure(command, output, returncode)
                if _setup_hint:
                    result_dict["setup_hint"] = _setup_hint
