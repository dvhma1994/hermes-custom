...
DANGEROUS_PATTERNS = [
    ...
    # checkout/restore of the WORKTREE overwrites tracked files with HEAD/index,
    # irreversibly discarding uncommitted edits — same data-loss class as
    # `git reset --hard`. Gated only for the discarding forms; branch switches
    # (`checkout -b`, `checkout <branch>`) and unstaging (`restore --staged`) are
    # left un-gated. `_normalize_command_for_detection` lowercases first.
    (r'\bgit\s+checkout\s+(?:[^\s]+\s+)*--\s', "git checkout -- <path> (discards uncommitted changes)"),
    (r'\bgit\s+checkout\s+\.(?:\s|$)', "git checkout . (discards all uncommitted changes)"),
    (r'\bgit\s+checkout\s+[^\n]*\s\.(?:\s|$)', "git checkout <tree> . (discards uncommitted changes)"),
    (r'\bgit\s+checkout\s+(?:-f\b|--force\b)', "git checkout --force (discards uncommitted changes)"),
    (r'\bgit\s+switch\b[^\n]*--discard-changes\b', "git switch --discard-changes (discards uncommitted changes)"),
    # restore: gate worktree-discarding forms. Plain `restore <path>`/`.` and any
    # `--worktree` discard; `--staged` (without --worktree) only unstages — safe.
    (r'\bgit\s+restore\b(?![^\n]*(?:--staged|--source))', "git restore (discards uncommitted worktree changes)"),
    (r'\bgit\s+restore\b[^\n]*--worktree\b', "git restore --worktree (discards uncommitted worktree changes)"),
    (r'\bgit\s+stash\s+(?:drop|clear)\b', "git stash drop/clear (permanently deletes stashed changes)"),
