...
def skill_search(query: str = "", limit: int = 5, db=None) -> str:
    ...
    try:
        ...
        if sr.is_enabled():
            # Use the caller's db when provided; otherwise fall back to the default
            # session DB's sidecar (tool handlers are not always handed a db object,
            # which would otherwise force keyword-only mode live).
            rec = sr.SemanticRecall.for_db(db) if db is not None else sr.SemanticRecall.for_default()
            if rec.available:
