...
# Subagent file-write audit (HERMES_SUBAGENT_AUDIT): a child summary that CLAIMS a
# file change while the harness recorded NO write for it is a likely fabricated
# success. Conservative: needs both a write-verb AND a path-ish token, so "I
# analyzed the code" or "the tests pass" don't trip it.
_FILE_CLAIM_VERB_RE = re.compile(
    r"(?i)\b(wrote|writing|created|creating|added|adding|modified|modifying|"
    r"updated|updating|patched|patching|implemented|implementing|edited|editing|"
    r"refactored|refactoring)\b"
)
# A path separator OR a known source/config extension — so version/number tokens
# like "v2.0" or "3.5x" don't read as filenames.
_FILE_CLAIM_PATH_RE = re.compile(
    r"(?i)(?:[\w.-]+[/\\][\w./\\-]+"
    r"|\b[\w-]+\.(?:py|js|ts|tsx|jsx|go|rs|rb|java|kt|php|c|h|cc|cpp|hpp|cs|swift|"
    r"json|ya?ml|toml|ini|cfg|md|rst|txt|sh|bash|zsh|sql|html?|css|scss|xml|"
    r"proto|gradle|lock|env|dockerfile)\b)"
)


def _summary_claims_file_change(summary: str) -> bool:
    if not summary:
        return False
    return bool(_FILE_CLAIM_VERB_RE.search(summary) and _FILE_CLAIM_PATH_RE.search(summary))
    ...
    **_kwargs,
    ...
    try:
        ...
        try:
            ...
            _files_read = []
        # NOTE: must use writes_by (no path filter), NOT writes_since(..., []) —
        # the latter gates on `p in set(paths)` so an empty allowlist always
        # returns {} (this attribution, and the TUI's, was silently dead before).
        try:
            _files_written = file_state.writes_by(child_task_id, wall_start)[:40]
        except Exception:
            _files_written = []

        # Ground-truth file-write attribution for the PARENT MODEL (not just the
        # TUI overlay): the parent otherwise sees only the child's self-reported
        # summary, so a child that ran out of budget / edited the wrong path /
        # only "planned" is relayed as completed with no machine signal. Surfacing
        # what it ACTUALLY wrote — plus a warning when a write CLAIM has no recorded
        # write — lets the parent catch a fabricated success. Gated on
        # HERMES_SUBAGENT_AUDIT; additive (extra entry fields the model sees) and
        # reversible.
        if os.getenv("HERMES_SUBAGENT_AUDIT") == "1":
            entry["files_written"] = _files_written
            entry["files_read"] = _files_read
            # Only write_file/patch record in file_state; a child that wrote via
            # the terminal (echo/heredoc) records nothing — so warn only when it
            # used NEITHER the file tools NOR the terminal yet claims a change.
            _used_terminal = any(
                isinstance(t, dict) and t.get("tool") == "terminal" for t in tool_trace
            )
            if (
                status == "completed"
                and not _files_written
                and not _used_terminal
                and _summary_claims_file_change(summary)
            ):
                entry["verification_warning"] = (
                    "This subagent's summary claims a file change, but it made no "
                    "tracked file write (no write_file/patch, no terminal). Treat its "
                    "success as UNVERIFIED — inspect the actual file(s) before "
                    "relaying this as done."
                )
...
def record_delegate_outcome_safe(parent_agent, results: list) -> None:
    """Milestone 1 helper: record delegate outcome to RuntimeFeedback.

    Extracted as a pure function for testability. Failures are swallowed.
    """
    try:
        collector = getattr(parent_agent, "_runtime_feedback", None)
        ...
        if not isinstance(collector, RuntimeFeedbackCollector):
            return
        any_error = any(
            entry.get("status") in {"error", "interrupted"} for entry in results
        )
        collector.record_delegate_outcome(
            {
                "tool_name": "delegate_task",
                "outcome": "failure" if any_error else "success",
                "task_count": len(results),
                "timestamp": time.time(),
            }
        )
    except Exception:
        logger.debug("Runtime feedback delegate-outcome recording failed", exc_info=True)
...
def delegate_task(
    ...
    # Milestone 1: record delegate outcome to RuntimeFeedback if available.
    record_delegate_outcome_safe(parent_agent, results)
